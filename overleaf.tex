...
\begin{itemize}
    ...
    \item three $\Rr$-divisors $B_Y,L_Y$ and $H_Y$ on $Y$,\wang{If $Y$ is not $\mathbb{Q}$-factorial, then how to show $L_{Y}$ and $H_{Y}$ are $\mathbb{R}$-Cartier divisor?}
...
By Lemma \ref{lem: extracting divisor},\wang{We need $(X,D(s+\epsilon),\Mm)$ to be lc. We only have $(X,D(s),\Mm)$ lc.} there is an extraction $g: V\rightarrow X$ of $E$ such that $V$ is $\Qq$-factorial.  We let $B_V,L_V,H_V$ be the birational transforms of $B_W,L_W$ and $H_W$ on $V$ respectively, then we have
\begin{align*}
